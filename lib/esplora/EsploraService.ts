...
import type { Network, TxStatus, Utxo } from "@dgen/esplora-types";
...
const sanitizeCacheKey = (cacheKey: string): string => {
...
const trackInFlight = (cacheKey: string, promise: Promise<any>): void => {
  ...
  if (inFlightRequests.size >= MAX_INFLIGHT_ENTRIES) {
    ...
    console.warn(
      `[Esplora] Skipping in-flight tracking for ${safeKey}; map at ${inFlightRequests.size}`
    );
    return;
  }
...
export class EsploraService {
  ...
  private async getTipGateMarker(cacheKey: string, statKey: string): Promise<string | null> {
    try {
      const marker = await db.get(`${cacheKey}:tip`);
      return marker ?? null;
    } catch (error) {
      console.warn(`[Esplora] Tip gate marker read error for ${statKey}:`, error);
      return null;
    }
  }

  private async setTipGateMarker(
    cacheKey: string,
    statKey: string,
    tipHash: string,
    ttlSeconds: number
    ...
    try {
      ...
      console.warn(`[Esplora] Tip gate marker write error for ${statKey}:`, error);
    }
  }

  private async clearTipGateMarker(cacheKey: string, statKey: string): Promise<void> {
    try {
      await db.del(`${cacheKey}:tip`);
    } catch (error) {
      console.warn(`[Esplora] Tip gate marker delete error for ${statKey}:`, error);
    }
  }
  ...
  private async tryTipGate<T>(cacheKey: string, statKey: string): Promise<T | null> {
    ...
    const marker = await this.getTipGateMarker(cacheKey, statKey);
  ...
  private async fetchWithRetryInternal<T, C>(
    url: string,
    cacheKey: string,
    ttl: number,
    options: {
    ...
    // Check cache first
    const cachedData = await this.getFromCache<C>(cacheKey);
    if (cachedData !== null) {
      this.bumpStat(statKey, "cacheHits");
      return options.fromCache(cachedData);
    }

    if (options.useTipGate) {
    ...
    if (Date.now() < backoffState.backoffUntil) {
      const staleData = await this.getFromStaleCache<C>(cacheKey);
      if (staleData !== null) {
        console.log(`[Esplora] In backoff, returning stale data for ${statKey}`);
        this.bumpStat(statKey, "staleHits");
        return options.fromCache(staleData);
      }
    ...
    if (existingRequest) {
      if (Date.now() - existingRequest.startedAt > MAX_INFLIGHT_AGE_MS) {
        ...
        console.log(`[Esplora] Deduplicating request for ${statKey}`);
    ...
    const requestPromise = this.executeRequestInternal<T, C>(
      url,
      cacheKey,
      ttl,
      maxRetries,
  ...
  private async fetchWithRetry<T>(
    ...
    return this.fetchWithRetryInternal<T, T>(url, cacheKey, ttl, {
      maxRetries,
  ...
  private async fetchTextWithRetry(
    ...
    return this.fetchWithRetryInternal<string, string>(url, cacheKey, ttl, {
      maxRetries,
      accept: "text/plain",
  ...
  private async fetchBufferWithRetry(
    ...
    return this.fetchWithRetryInternal<Uint8Array, number[]>(url, cacheKey, ttl, {
      maxRetries,
      accept: "application/octet-stream",
      parse: async (response) => new Uint8Array(await response.arrayBuffer()),
      fromCache: (cached) => Uint8Array.from(cached),
      toCache: (data) => Array.from(data),
    });
  }

  private async executeRequestInternal<T, C>(
    url: string,
    cacheKey: string,
    ttl: number,
    maxRetries: number,
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        const headers = await this.buildHeaders(options.accept);
        ...
        await this.setCache(cacheKey, options.toCache(data), ttl);
  ...
  async getTxStatus(txid: string, network: Network = "liquid"): Promise<TxStatus> {
    const cacheKey = `esplora:tx:${network}:${txid}`;
  ...
  async getTx(txid: string, network: Network = "liquid"): Promise<any> {
    const cacheKey = `esplora:txfull:${network}:${txid}`;
  ...
  async getTxHex(txid: string, network: Network = "liquid"): Promise<string> {
    const cacheKey = `esplora:txhex:${network}:${txid}`;
  ...
  async getTxRaw(txid: string, network: Network = "liquid"): Promise<Uint8Array> {
    const cacheKey = `esplora:txraw:${network}:${txid}`;
  ...
  async getAddressUtxos(address: string, network: Network = "liquid"): Promise<Utxo[]> {
    const cacheKey = `esplora:utxo:${network}:${address}`;
  ...
  async getScripthashUtxos(scripthash: string, network: Network = "liquid"): Promise<Utxo[]> {
    const cacheKey = `esplora:scripthash_utxo:${network}:${scripthash}`;
    ...
    if (this.isConfirmedOnlyUtxos(utxos)) {
      ...
      const tipHash = await this.getCachedTipHash(network);
      if (tipHash) {
        const tipGateTtl = Math.max(
          CACHE_TTL.UTXO_CONFIRMED * STALE_TTL_MULTIPLIER,
          CACHE_TTL.UTXO_CONFIRMED
        );
        await this.setTipGateMarker(cacheKey, statKey, tipHash, tipGateTtl);
      }
    } else {
      const statKey = this.getStatKey(cacheKey);
      await this.clearTipGateMarker(cacheKey, statKey);
    }
  ...
  async getTipHeight(network: Network = "liquid"): Promise<number> {
    ...
    const heightText = (await this.fetchTextWithRetry(
      url,
      cacheKey,
  ...
  async getAddressTxs(
    ...
    const cacheKey = lastSeenTxid
      ? `esplora:addr_txs:${network}:${address}:${lastSeenTxid}`
      : `esplora:addr_txs:${network}:${address}`;
    ...
    if (this.isConfirmedOnlyTxs(txs as EsploraTx[])) {
      ...
      if (tipHash) {
        const tipGateTtl = Math.max(
          CACHE_TTL.TXS_LIST_CONFIRMED * STALE_TTL_MULTIPLIER,
          CACHE_TTL.TXS_LIST_CONFIRMED
        );
        await this.setTipGateMarker(cacheKey, statKey, tipHash, tipGateTtl);
      }
    } else {
  ...
  async getScripthashTxs(
    ...
    const cacheKey = lastSeenTxid
      ? `esplora:scripthash_txs:${network}:${scripthash}:${lastSeenTxid}`
      : `esplora:scripthash_txs:${network}:${scripthash}`;
  ...
  async getScripthashTxsConfirmed(
    ...
    const cacheKey = `esplora:scripthash_txs_confirmed:${network}:${scripthash}`;
    const url = `${this.getBaseUrl(network)}/scripthash/${scripthash}/txs/chain`;

    const txs = await this.fetchWithRetry<any[]>(
      url,
      cacheKey,
      CACHE_TTL.TXS_LIST_CONFIRMED
    );
  ...
  async getScripthashTxsMempool(
    ...
    const cacheKey = `esplora:scripthash_txs_mempool:${network}:${scripthash}`;
  ...
  async getAddressTxsConfirmed(
    ...
    const cacheKey = `esplora:addr_txs_confirmed:${network}:${address}`;
  ...
  async getAddressTxsMempool(
    ...
    const cacheKey = `esplora:addr_txs_mempool:${network}:${address}`;
  ...
  async getBlockHeader(blockHash: string, network: Network = "liquid"): Promise<string> {
    const cacheKey = `esplora:block_header:${network}:${blockHash}`;
