...
import { g, s } from "./db";
// import { lnb } from "./ln"; // TODO: eCash disabled until browser wallet integration
import { fail, wait } from "./utils";
import {
  CashuMint,
  CashuWallet,
  MintQuoteState,
...
const m = new CashuMint(getConfig().mintUrl);
const w = new CashuWallet(m);

const enc = (proofs) =>
  getEncodedToken({
    mint: getConfig().mintUrl,
    proofs,
  });
