...
import { scan } from "./strfry";
import { fail, fields, getUser, pick } from "./utils";
...
const relay = config.nostr ? await Relay.connect(config.nostr) : null;

export const serverSecret = config.nostrKey ? bytesToHex(
  (nip19.decode(config.nostrKey).data as any) as Uint8Array,
) : "";

export const serverPubkey = serverSecret ? getPublicKey(serverSecret) : "";
...
export const anon = (pubkey: string) => ({
...
export async function publish(ev, url = getConfig().nostr) {
...
export const getCount = async (pubkey) => {
  try {
    ...
    if (followers === null) {
      // [followers] = await count({ // Nostr disabled
      //   "#p": [pubkey],
      //   kinds: [3],
      // });
      followers = 0;
    ...
    warn("Failed to get follow count:", e.message);
  }
};
...
export const getNostrUser = async (key) => {
  ...
  // Fetch balance from Redis if user has an id
  if (user.id) {
