...
docker compose up -d
docker run -it -v $(pwd):/home/bun/app --entrypoint bun asoltys/dgen-server i
docker exec -it bc bitcoin-cli createwallet dgen
...
docker exec -it lq elements-cli createwallet dgen
